...
import type { CommitDetails, CommitStat, CommitSummary, ListOptions, TimeWindow } from "../../shared/types/github.js";
...
export async function listCommits(
    ...
    const commitDetails = await listCommitsDetail(octokit, org, repo, branch, window, opts);
    ...
    async function worker() {
        while (true) {
            ...
            try {
                const sum = await getCommitStat(octokit, org, repo, item);
 ...
 */
async function listCommitsDetail(
    octokit: Octokit,
    org: string,
    repo: string,
    branch: string,
 ...
 */
function removeDuplicateCommits(
  items: CommitDetails[]
): CommitDetails[] {
